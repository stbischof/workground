...
public abstract class AbstractDbMappingSchemaModifier implements DatabaseMappingSchemaProvider {
    ...
    protected MappingSchema modifyMappingSchema(MappingSchema mappingSchemaOriginal) {
        ...
        if (mappingSchemaOriginal != null) {
            ...
            MappingPhysicalSchema physicalSchema = schemaPhysicalSchema(mappingSchemaOriginal);
    ...
    private MappingColumnDef columnDef(MappingColumnDef columnDef) {
        if (columnDef != null) {
            ...
            String internalType = columnDefInternalType(columnDef);
            return new_ColumnDef(timeDomain, name, type, internalType);
        }
        return null;
    }
    ...
    protected abstract MappingColumnDef new_ColumnDef(MappingTimeDomain timeDomain, String name, TypeEnum type, String internalType);

    protected TypeEnum columnDefType(MappingColumnDef columnDef) {
        return columnDef.type();
    }

    protected String columnDefInternalType(MappingColumnDef columnDef) {
        return columnDef.internalType();
    }
    ...
    protected String expressionName(MappingExpression expression) {
        return expression.name();
    }

    protected String expressionTable(MappingExpression expression) {
    ...
    protected MappingPhysicalSchema schemaPhysicalSchema(MappingSchema schema) {
        if (schema != null) {
            List<MappingPhysicalSchemaElement> childrens = schemaPhysicalSchemaElements(schema);
            return new_MappingSchema(childrens);
        }
        return null;
    }

    protected List<MappingPhysicalSchemaElement> schemaPhysicalSchemaElements(MappingSchema schema) {
        return schemaPhysicalSchemaElements(schema.physicalSchema());
    }

    protected List<MappingPhysicalSchemaElement> schemaPhysicalSchemaElements(MappingPhysicalSchema physicalSchema) {
        if (physicalSchema != null) {
            return schemaPhysicalSchemaElements(physicalSchema.children());
        }
        return null;
    }

    protected List<MappingPhysicalSchemaElement> schemaPhysicalSchemaElements(List<MappingPhysicalSchemaElement> list) {
        if (list != null) {
            return list.stream().map(this::physicalSchemaElement).toList();
        }
        return null;
    }

    private MappingPhysicalSchemaElement physicalSchemaElement(MappingPhysicalSchemaElement physicalSchemaElement) {
        if (physicalSchemaElement != null) {
            if (physicalSchemaElement instanceof MappingAutoGeneratedDateTable autoGeneratedDateTable) {
                List<MappingTableElement> tableElements = autoGeneratedDateTableTableElements(autoGeneratedDateTable);
                Instant startDate = autoGeneratedDateTableStartDate(autoGeneratedDateTable);
                Instant endDate = autoGeneratedDateTableEndDate(autoGeneratedDateTable);
                String name = autoGeneratedDateTableName(autoGeneratedDateTable);
                String schema = autoGeneratedDateTableSchema(autoGeneratedDateTable);
                String alias = autoGeneratedDateTableAlias(autoGeneratedDateTable);
                String keyColumn = autoGeneratedDateTableKeyColumn(autoGeneratedDateTable);
                return new_AutoGeneratedDateTable(tableElements,
                    startDate,
                    endDate,
                    name,
                    schema,
    ...
    protected String autoGeneratedDateTableKeyColumn(MappingAutoGeneratedDateTable autoGeneratedDateTable) {
        if (autoGeneratedDateTable != null) {
            return autoGeneratedDateTable.keyColumn();
        }
        return null;
    }

    protected String autoGeneratedDateTableAlias(MappingAutoGeneratedDateTable autoGeneratedDateTable) {
        if (autoGeneratedDateTable != null) {
            return autoGeneratedDateTable.alias();
        }
        return null;
    }

    protected String autoGeneratedDateTableSchema(MappingAutoGeneratedDateTable autoGeneratedDateTable) {
        if (autoGeneratedDateTable != null) {
            return autoGeneratedDateTable.schema();
        }
        return null;
    }

    protected String autoGeneratedDateTableName(MappingAutoGeneratedDateTable autoGeneratedDateTable) {
        if (autoGeneratedDateTable != null) {
            return autoGeneratedDateTable.name();
        }
        return null;
    }

    protected  Instant autoGeneratedDateTableEndDate(MappingAutoGeneratedDateTable autoGeneratedDateTable) {
        if (autoGeneratedDateTable != null) {
            return autoGeneratedDateTable.endDate();
        }
        return null;
    }

    protected Instant autoGeneratedDateTableStartDate(MappingAutoGeneratedDateTable autoGeneratedDateTable) {
        if (autoGeneratedDateTable != null) {
            return autoGeneratedDateTable.startDate();
        }
        return null;
    }

    protected  List<MappingTableElement> autoGeneratedDateTableTableElements(MappingAutoGeneratedDateTable autoGeneratedDateTable) {
        return autoGeneratedDateTableTableElements(autoGeneratedDateTable.tableElement());
    }

    protected  List<MappingTableElement> autoGeneratedDateTableTableElements(List<MappingTableElement> list) {
        if (list != null) {
            return list.stream().map(this::tableElement).toList();
        }
        return null;
    }

    private MappingTableElement tableElement(MappingTableElement tableElement) {
        if (tableElement != null) {
            if (tableElement instanceof MappingColumnDefs columnDefs) {
                List<RealOrCalcColumnDef> columns = columnDefsColumns(columnDefs);
                return new_ColumnDefs(columns);
            }
            if (tableElement instanceof MappingHints hints) {
                List<MappingHint> hs = hintsHints(hints);
                return new_Hints(hs);
            }
            if (tableElement instanceof MappingKey key) {
                List<MappingColumn> columns = keyColumns(key);
                String name = keyName(key);
                return new_Key(columns, name);
            }
        }
        return null;
    ...
    protected List<RealOrCalcColumnDef> columnDefsColumns(MappingColumnDefs columnDefs) {
        if (columnDefs != null) {
            return columnDefsColumns1(columnDefs.columns());
        }
        return null;
    }

    protected List<RealOrCalcColumnDef> columnDefsColumns1(List<RealOrCalcColumnDef> list) {
        if (list != null) {
            list.stream().map(this::realOrCalcColumnDef).toList();
        }
        return null;
    }

    private RealOrCalcColumnDef realOrCalcColumnDef(RealOrCalcColumnDef realOrCalcColumnDef) {
        if (realOrCalcColumnDef != null) {
            String name = realOrCalcColumnDefName(realOrCalcColumnDef);
            TypeEnum type = realOrCalcColumnDefType(realOrCalcColumnDef);
            String internalType = realOrCalcColumnDefInternalType(realOrCalcColumnDef);
            if (realOrCalcColumnDef instanceof MappingCalculatedColumnDef calculatedColumnDef) {
                MappingExpression expression = calculatedColumnDefExpression(calculatedColumnDef);
                return new_CalculatedColumnDef(expression, name, type, internalType);
            }
            if (realOrCalcColumnDef instanceof MappingColumnDef columnDef) {
                MappingTimeDomain timeDomain = columnDefTimeDomain(columnDef);
                return new_ColumnDef(timeDomain, name, type, internalType);
            }
        }
    ...
    protected MappingExpression calculatedColumnDefExpression(MappingCalculatedColumnDef calculatedColumnDef) {
        if (calculatedColumnDef != null) {
            return  expressionExpression(calculatedColumnDef.expression());
        }
        return null;
    }

    protected MappingExpression expressionExpression(MappingExpression expression) {
        if (expression != null) {
            if (expression instanceof MappingColumn column) {
                String table = expressionTable(column);
                String name = expressionName(column);
                String genericExpression = columnGenericExpression(column);
                return new_Column(table, name, genericExpression);
            }
            if (expression instanceof MappingExpressionView expressionView) {
                List<MappingSQL> sqls = expressionViewSqls(expressionView);
                String table = expressionViewTable(expressionView);
                String name = expressionViewName(expressionView);
                return new_ExpressionView(sqls, table, name);
            }
        }
        return null;
    ...
    protected List<MappingSQL> expressionViewSqls(MappingExpressionView expressionView) {
        if (expressionView != null) {
            return expressionViewSqls(expressionView.sqls());
        }
        return null;
    }

    protected List<MappingSQL> expressionViewSqls(List<MappingSQL> list) {
        if (list != null) {
            return list.stream().map(this::sql).toList();
        }
        return null;
    }

    protected String expressionViewTable(MappingExpressionView expressionView) {
        if (expressionView != null) {
            return expressionView.name();
        }
        return null;
    }


    protected String expressionViewName(MappingExpressionView expressionView) {
        if (expressionView != null) {
            return expressionView.name();
        }
        return null;
    ...
    protected String realOrCalcColumnDefInternalType(RealOrCalcColumnDef realOrCalcColumnDef) {
        if (realOrCalcColumnDef != null) {
            return realOrCalcColumnDef.internalType();
        }
        return null;
    }

    protected TypeEnum realOrCalcColumnDefType(RealOrCalcColumnDef realOrCalcColumnDef) {
        if (realOrCalcColumnDef != null) {
            return realOrCalcColumnDef.type();
        }
        return null;
    }

    protected String realOrCalcColumnDefName(RealOrCalcColumnDef realOrCalcColumnDef) {
        if (realOrCalcColumnDef != null) {
            return realOrCalcColumnDef.name();
        }
        return null;
    }

    private List<MappingHint> hintsHints(MappingHints hints) {
        if (hints != null) {
            return hintsHints(hints.hints());
        }
        return null;
    }

    private List<MappingHint> hintsHints(List<MappingHint> list) {
        if (list != null) {
            list.stream().map(this::hint).toList();
        }
        return null;
    }

    private List<MappingColumn> keyColumns(MappingKey key) {
        if (key != null) {
            return keyColumns(key.columns());
        }
        return null;
    }

    private List<MappingColumn> keyColumns(List<MappingColumn> columns) {
        if (columns != null) {
            columns.stream().map(this::column).toList();
        }
        return null;
    }

    private MappingColumn column(MappingColumn column) {
        if (column != null) {
            String table = expressionTable(column);
            String name = expressionName(column);
            String genericExpression = columnGenericExpression(column);
    ...
    protected String columnGenericExpression(MappingColumn column) {
       if (column != null) {
           column.genericExpression();
       }
       return null;
    }

    private String keyName(MappingKey key) {
        if (key != null) {
            return key.name();
        }
        return null;
    ...
    protected abstract MappingPhysicalSchemaElement new_AutoGeneratedDateTable(List<MappingTableElement> tableElements,
                                                                               Instant startDate, Instant endDate,
                                                                               String name, String schema,
                                                                               String alias, String keyColumn);

    protected abstract MappingTableElement new_ColumnDefs(List<RealOrCalcColumnDef> columns);

    protected abstract MappingTableElement new_Hints(List<MappingHint> hs);

    protected abstract MappingTableElement new_Key(List<MappingColumn> columns, String name);

    protected abstract MappingColumn new_Column(String table, String name, String genericExpression);

    protected abstract MappingPhysicalSchema new_MappingSchema(List<MappingPhysicalSchemaElement> childrens);

    protected abstract RealOrCalcColumnDef new_CalculatedColumnDef(MappingExpression expression, String name, TypeEnum type, String internalType);
}
