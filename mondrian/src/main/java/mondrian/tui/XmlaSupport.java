...
public class XmlaSupport {
    private static final String XMLA_TRANSFORM_LEFT =
        "<?xml version='1.0'?>"
            + "<xsl:stylesheet "
            ...
            + "  xmlns:";
    private static final String XMLA_TRANSFORM_MIDDLE1 =
        "='urn:schemas-microsoft-com:xml-analysis'"
            ...
            private static final String XMLA_TRANSFORM_MIDDLE2 =
            ":DiscoverResponse'> "
            + "  <xsl:choose> "
            + "    <xsl:when test=\"$soap='none'\"> "
            + "      <xsl:apply-templates/> "
    ...
    private static final String XMLA_TRANSFORM_RIGHT =
                ":return'> "
            + "  <xsl:choose> "
            + "    <xsl:when test=\"$soap='none'\"> "
            + "      <xsl:apply-templates/> "
    ...
    public static String getXmlaTransform(String xmlaPrefix) {
        return
            new StringBuilder(XMLA_TRANSFORM_LEFT)
                .append(xmlaPrefix)
                .append(XMLA_TRANSFORM_MIDDLE1)
                .append(xmlaPrefix)
                .append(XMLA_TRANSFORM_MIDDLE2)
                .append(xmlaPrefix)
                .append(XMLA_TRANSFORM_RIGHT)
                .toString();
    }
        ...
        throws SAXException, IOException
    {
        ...
        String schemaLocationPropertyValue = new StringBuilder(xmlns).append(' ').append("xmlschema").toString();
               ...
               TransformerException
    {
        ...
        if (! schemaDoc.hasChildNodes()) {
            LOGGER.debug(
                "XmlaSupport.validateEmbeddedSchema: schemaDoc has no children");
            return false;
        }
